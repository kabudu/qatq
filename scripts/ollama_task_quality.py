...
def main() -> int:
    ...
    run([qatq_bin, "encode", "--mode", "qatq-exact", docs_f32le, docs_qatq])
...
def run_relevance_score_task(args, qatq_bin: Path, work_dir: Path, report_path: Path, started: float, embedding_error: str) -> int:
    ...
    run([qatq_bin, "fixture", "verify", "--manifest", manifest, "--output", audit])
    run([qatq_bin, "encode", "--mode", "qatq-exact", scores_f32le, scores_qatq])
...
def render_report(
    ...
    lines = [
        ...
        "It proves that QATQ exact transport preserves this model task after",
    ...
    lines.extend([
        ...
        "## Claim Boundary",
        "",
        "- Supported: QATQ exact transport preserves this local model-output retrieval task.",
...
def render_score_report(
    ...
    lines = [
        ...
        "the QATQ fixture path and verified QATQ exact transport. The installed",
    ...
    lines.extend([
        ...
        "## Claim Boundary",
        "",
        "- Supported: QATQ exact transport preserves this local model-output task tensor and its top-1 retrieval decisions.",
