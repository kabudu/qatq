...
fn run() -> Result<(), String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--stride-elements" => {
                let value = args
                    .get(index + 1)
                    .ok_or_else(|| "--stride-elements requires a value".to_string())?;
                stride_elements = Some(
                    value
                        .parse::<usize>()
                        .map_err(|error| format!("invalid --stride-elements: {error}"))?,
                );
                index += 2;
            }
            "-h" | "--help" => {
    ...
    for input in &inputs {
        rows.push(bench_input(input, iters, stride_elements)?);
    }
    let report = render_report(&rows, iters, stride_elements);
...
fn bench_input(
    ...
    let encode = || {
        if let Some(stride) = stride_elements
            && input.dtype != TensorDType::F32
        {
            try_encode_qatq_exact_tensor_le_with_stride_hint(&bytes, input.dtype, stride)
        } else {
            try_encode_qatq_exact_tensor_le(&bytes, input.dtype)
        }
    };
    let qatq =
        encode().map_err(|error| format!("QATQ encode failed for {}: {error}", input.label))?;
    ...
    let qatq_encode = time_repeated(iters, || encode().expect("QATQ encode"));
...
fn render_report(rows: &[BenchRow], iters: usize, stride_elements: Option<usize>) -> String {
...
fn print_usage() {
    eprintln!("usage:");
    eprintln!(
        "  qatq-kv-bench --dir <kv-export-dir> [--stride-elements N] [--iters N] [--output report.md]"
    );
    eprintln!(
        "  qatq-kv-bench --input <label>:<f32|f16|bf16>:<path> [...] [--stride-elements N] [--output report.md]"
    );
}
