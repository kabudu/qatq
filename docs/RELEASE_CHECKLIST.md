...
uses it for package history, but QATQ does not publish the generated changelog
...
release title as a body heading, paste the raw changelog, or duplicate the
automatically attached asset list.

Do not hard-wrap prose in a GitHub Release notes source. Each paragraph and
each list item must occupy one source line; let the browser wrap it to the
available viewport. Use source newlines only for intentional Markdown block
boundaries. GitHub release rendering can preserve soft source newlines and make
80-column wrapping appear as arbitrary line breaks.

Preview the rendered Markdown on GitHub and inspect it at desktop and narrow
widths before tagging. Confirm heading hierarchy, list indentation, code fences,
link targets, line wrapping, and the absence of placeholder or internal-only
text. The release workflow fails closed if the versioned notes file is absent,
its H1 does not match the tag, its body is empty, or prose/list items are
hard-wrapped. Run the same validation locally:
...
cargo-dist and intentionally customised. cargo-dist builds and uploads the
artifacts; the workflow takes the public title/body only from the reviewed
`.github/release-notes/vX.Y.Z.md` file, then publishes:
