...
fn run() -> Result<(), String> {
    ...
    if should_benchmark {
        report = render_benchmark_report(&benchmarked, &options);
    }
...
struct QualityDataset {
    ...
    probe_count: usize,
    turboquant_ratio: f64,
    phase1_ratio: f64,
    turboquant_qjl: QualityMetrics,
...
fn quality_dataset(dataset: &Dataset) -> Result<QualityDataset, String> {
    ...
    let turboquant_encoded = encode_turboquant_q4(&finite_values);
    let phase1_encoded = encode_phase1_q4(&finite_values);
    let phase1_decoded = decode_phase1_q4(&phase1_encoded).map_err(|error| error.to_string())?;
    let probes = quality_query_probes(&finite_values);

    let mut turboquant_errors = Vec::with_capacity(probes.len());
    let mut phase1_errors = Vec::with_capacity(probes.len());
    for query in &probes {
        let exact = inner_product(&finite_values, query);
        let turboquant_estimated = estimate_turboquant_q4_inner_product(&turboquant_encoded, query)
            .map_err(|error| error.to_string())?;
        let phase1_estimated = inner_product(&phase1_decoded, query);
        turboquant_errors.push((turboquant_estimated - exact).abs() as f64);
        phase1_errors.push((phase1_estimated - exact).abs() as f64);
    }
    ...
    Ok(QualityDataset {
        ...
        turboquant_ratio: compression_ratio(turboquant_encoded.len(), finite_values.len()),
        phase1_ratio: compression_ratio(phase1_encoded.len(), finite_values.len()),
...
fn quality_query_probes(values: &[f32]) -> Vec<Vec<f32>> {
    ...
    let mut self_probe = values.to_vec();
    normalize_query(&mut self_probe);

    let mut alternating = (0..count)
        .map(|index| if index % 2 == 0 { 1.0 } else { -1.0 })
        .collect::<Vec<_>>();
    normalize_query(&mut alternating);

    let mut sinusoidal = (0..count)
        .map(|index| {
            let x = index as f32;
            (x * 0.071).sin() + (x * 0.017).cos() * 0.5
        })
        .collect::<Vec<_>>();
    normalize_query(&mut sinusoidal);

    let stride = (count / 16).max(1);
    let mut sparse = vec![0.0; count];
    for (slot, value) in sparse.iter_mut().step_by(stride).enumerate() {
        *value = if slot % 2 == 0 { 1.0 } else { -1.0 };
    }
    normalize_query(&mut sparse);

    vec![self_probe, alternating, sinusoidal, sparse]
}

fn normalize_query(values: &mut [f32]) {
    let norm = values
        .iter()
...
fn quality_metrics(errors: &[f64]) -> QualityMetrics {
    if errors.is_empty() {
...
fn render_quality_summary(quality: &[QualityDataset]) -> String {
    let mut out = String::new();
    out.push_str("# QATQ Quality Experiments\n\n");
    out.push_str("Generated by `qatq-bench --quality-output`. These paper-facing codec probes compare the base `turboquant-q4` QJL inner-product estimator with the quaternion-overlay `phase1-q4` decoded inner product on deterministic finite query probes.\n\n");
    out.push_str("This is not a model-quality or perplexity result. It is a deterministic codec-level proxy for query/dot-product preservation over the selected fixture corpus.\n\n");
    out.push_str("## Method\n\n");
    out.push_str("- Each dataset is evaluated with four deterministic query probes: normalized self, alternating signs, sinusoidal, and sparse signs.\n");
    out.push_str("- Non-finite source values are masked to zero for this quality-only probe so NaN/Inf exactness stress fixtures do not poison finite inner-product metrics.\n");
    out.push_str("- `turboquant-q4` is scored through `estimate_turboquant_q4_inner_product`, which uses the stored QJL residual signs.\n");
    out.push_str("- `phase1-q4` is scored by decoding the quaternion-overlay payload and taking the decoded inner product.\n\n");

    out.push_str("## Inner-Product Error Summary\n\n");
    out.push_str("| group | dataset | values | non-finite masked | probes | turboquant ratio | phase1 ratio | ratio delta | turboquant QJL MAE | phase1 decoded MAE | MAE delta | turboquant QJL RMSE | phase1 decoded RMSE | RMSE delta | winner |\n");
    out.push_str("| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |\n");
    for row in quality {
        let mae_delta = row.phase1_decoded.mean_abs_error - row.turboquant_qjl.mean_abs_error;
        let rmse_delta = row.phase1_decoded.rmse - row.turboquant_qjl.rmse;
        let winner = if row.phase1_decoded.mean_abs_error < row.turboquant_qjl.mean_abs_error {
            "phase1-q4"
        } else if row.turboquant_qjl.mean_abs_error < row.phase1_decoded.mean_abs_error {
            "turboquant-q4"
        } else {
            "tie"
        };
        out.push_str(&format!(
            ...
            row.name,
            row.value_count,
            row.non_finite_masked,
            row.probe_count,
            row.turboquant_ratio,
            row.phase1_ratio,
            row.phase1_ratio - row.turboquant_ratio,
            row.turboquant_qjl.mean_abs_error,
            row.phase1_decoded.mean_abs_error,
            mae_delta,
            row.turboquant_qjl.rmse,
            row.phase1_decoded.rmse,
            rmse_delta,
            winner
        ));
    }
    ...
    out.push_str("\n## Max Error Guardrail\n\n");
    out.push_str("| group | dataset | turboquant QJL max abs error | phase1 decoded max abs error | max-error delta |\n");
    out.push_str("| --- | --- | ---: | ---: | ---: |\n");
    for row in quality {
        out.push_str(&format!(
            "| {} | {} | {:.6} | {:.6} | {:+.6} |\n",
            row.group,
            row.name,
            row.turboquant_qjl.max_abs_error,
            row.phase1_decoded.max_abs_error,
            row.phase1_decoded.max_abs_error - row.turboquant_qjl.max_abs_error
        ));
    }

    out.push_str("\n## Use In Paper Drafts\n\n");
    out.push_str("- Use this table to discuss codec-level inner-product preservation, not downstream task quality.\n");
    out.push_str("- Positive deltas mean `phase1-q4` has larger error than the base `turboquant-q4` QJL estimator for this proxy; negative deltas mean the quaternion overlay is better for this proxy.\n");
    out.push_str(
