...
#[test]
fn live_vram_evidence_run_times_out_process_group() {
    let output = run_python_snippet(
        r#"
...
with tempfile.TemporaryDirectory() as raw_tmp:
    ...
    try:
        module.run([sys.executable, str(runner), str(marker)], cwd=Path.cwd(), timeout=1, capture=True)
...
with tempfile.TemporaryDirectory() as raw_tmp:
    ...
    assert plan["stop_after_min_passed_elapsed"] is False
    ...
    assert summary["gates"]["stop_after_min_passed_elapsed"] is False
...
#[test]
fn live_vram_server_burnin_stops_after_duration_target() {
    let output = run_python_snippet(
        r###"
import json, subprocess, sys, tempfile
from pathlib import Path
...
with tempfile.TemporaryDirectory() as raw_tmp:
    ...
    config.write_text(json.dumps({
        "defaults": {},
        "cases": [{"id": "duration-case", "model": str(model)}],
    }), encoding="utf-8")
...
""",
    ...
    result = subprocess.run(
        [
            ...
            "--timeout", "30",
            "--run-timeout", "120",
            "--min-passed-elapsed-seconds", "0.4",
    ...
    plan = json.loads((work_dir / "server-burnin-plan.json").read_text())
    assert plan["runs"] == 5
    assert plan["stop_after_min_passed_elapsed"] is True
    assert summary["status"] == "pass"
    assert summary["runs_requested"] == 5
    assert summary["runs_completed"] == 2
    assert summary["passed"] == 2
    assert summary["sustained_runtime"]["passed_elapsed_seconds"] >= 0.4
    assert summary["early_stop_reason"].startswith("duration target satisfied after 2 passing runs")
    assert summary["gates"]["stop_after_min_passed_elapsed"] is True
...
with tempfile.TemporaryDirectory() as raw_tmp:
    ...
    assert run["timeout_seconds"] == 1
    assert run["cleanup_signal"] == "SIGTERM"
    assert run["cleanup_escalated"] is False
    ...
    while not marker.exists() and time.time() < deadline:
        time.sleep(0.05)
    assert marker.read_text() == "sigterm"
    stderr = Path(run["stderr"]).read_text()
    assert "server burn-in matrix run exceeded timeout of 1s; cleanup=SIGTERM" in stderr
"#,
    );
...
assert "RESUME_EXISTING:" in workflow
assert "stop_after_min_passed_elapsed:" in workflow
assert "STOP_AFTER_MIN_PASSED_ELAPSED:" in workflow
...
#[test]
fn live_vram_hardware_counter_probe_timeout_cleans_process_group() {
    let output = run_python_snippet(
        r#"
import importlib.util, subprocess, sys, tempfile, time
from pathlib import Path
...
with tempfile.TemporaryDirectory() as raw_tmp:
    ...
    result = module.run_probe_command([sys.executable, str(runner), str(marker)], timeout=1)
    assert result.returncode == 124
...
spec = importlib.util.spec_from_file_location("runtime_kv", "scripts/llama_cpp_runtime_kv.py")
module = importlib.util.module_from_spec(spec)
sys.modules[spec.name] = module
spec.loader.exec_module(module)
...
with tempfile.TemporaryDirectory() as raw_tmp:
    ...
    try:
        module.run_command([sys.executable, str(runner), str(marker)], cwd=Path.cwd(), timeout=1)
...
#[test]
fn llama_cpp_kv_matrix_timeout_cleans_process_group() {
    let output = run_python_snippet(
        r#"
import importlib.util, subprocess, sys, tempfile, time
from pathlib import Path

spec = importlib.util.spec_from_file_location("kv_matrix", "scripts/llama_cpp_kv_matrix.py")
module = importlib.util.module_from_spec(spec)
sys.modules[spec.name] = module
spec.loader.exec_module(module)
