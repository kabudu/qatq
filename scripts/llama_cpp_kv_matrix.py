...
def main() -> int:
    ...
    parser.add_argument("--stride-elements", type=int, default=0)
    ...
    for index, (model, prompt, dtype, predict) in enumerate(cases, start=1):
        ...
        if args.stride_elements:
            bench_command.extend(["--stride-elements", str(args.stride_elements)])
        ...
        details.append((case_label, detail.replace(str(work_dir), "<matrix-work-dir>")))
...
def render_report(args, models, prompts, dtypes, predicts, rows, details, elapsed: float) -> str:
    ...
    out += "- llama-simple: `<patched-llama.cpp>/llama-simple`\n"
    out += f"- native stride hint: `{args.stride_elements or 'none'}`\n"
    out += f"- dtypes: `{', '.join(dtypes)}`\n"
    out += f"- token budgets: `{', '.join(str(p) for p in predicts)}`\n"
    out += f"- elapsed seconds: `{elapsed:.2f}`\n\n"
    out += "## Models\n\n"
    for model in models:
        out += f"- `{model.label}`: `{model.path.name}`\n"
    out += "\n## Prompts\n\n"
    for prompt in prompts:
